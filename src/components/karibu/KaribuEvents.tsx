...
function PastCard({ event }: { event: Event }) {
  ...
  return (
    <Link href={`/events/${event.slug}`} className="block opacity-90 transition-opacity hover:opacity-100">
      <div
        className="mb-2.5 h-[110px] rounded-[10px]"
        style={{
          backgroundImage:
            "repeating-linear-gradient(135deg, #EFE7D8 0 10px, #E7DDCB 10px 20px)",
        }}
      />
