...
export async function GET(
  ...
  const check = await checkApiPermission("ideas", "view")
  if (!check.authorized) return check.response

  const { id } = await params
  const item = await prisma.ideaSubmission.findUnique({ where: { id } })
...
export async function PATCH(
  ...
  const check = await checkApiPermission("ideas", "approve")
  if (!check.authorized) return check.response
  ...
  const existing = await prisma.ideaSubmission.findUnique({ where: { id } })
  if (!existing) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 })

  const updated = await prisma.ideaSubmission.update({
    where: { id },
    data: {
