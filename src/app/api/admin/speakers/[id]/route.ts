...
export async function GET(
  ...
  const check = await checkApiPermission("speakers", "view")
  if (!check.authorized) return check.response

  const { id } = await params
  const item = await prisma.speakerApplication.findUnique({ where: { id } })
...
export async function PATCH(
  ...
  const check = await checkApiPermission("speakers", "approve")
  if (!check.authorized) return check.response
  ...
  const existing = await prisma.speakerApplication.findUnique({ where: { id } })
  if (!existing) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 })

  const updated = await prisma.speakerApplication.update({
    where: { id },
    data: {
