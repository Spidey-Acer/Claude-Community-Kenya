      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link
            ...
            className="flex items-center gap-2 px-3 py-2 bg-[#00ff41]/10 hover:bg-[#00ff41]/20 border border-[#00ff41]/30 rounded text-[11px] font-mono text-[#00ff41] transition-all"
          >
          ...
          {events.length === 0 ? (
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[#141414]">
                {events.map((event) => (
                  <tr key={event.id} className="hover:bg-[#111] transition-colors cursor-pointer group">
                    <td className="px-4 py-3">
                      <Link href={`/admin/events/${event.id}`} className="block">
                        <div className="text-sm font-mono text-[#e0e0e0] group-hover:text-[#00ff41] transition-colors">{event.title}</div>
                        <div className="text-[11px] font-mono text-[#444]">/events/{event.slug}</div>
                      </Link>
                    </td>
                    <td className="px-4 py-3">
                      <Link href={`/admin/events/${event.id}`} className="block text-[11px] font-mono text-[#666]">{TYPE_LABELS[event.type] ?? event.type}</Link>
                    </td>
                    <td className="px-4 py-3">
                      <Link href={`/admin/events/${event.id}`} className="block text-[11px] font-mono text-[#666]">{event.city}</Link>
                    </td>
                    <td className="px-4 py-3">
                      <Link href={`/admin/events/${event.id}`} className="block">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/events/${event.id}`} className="block text-[11px] font-mono text-[#444]">{formatDate(event.date.toISOString())}</Link>
                    </td>
                    <td className="px-4 py-3">
                      <Link href={`/admin/events/${event.id}`} className="block">
