    ...
    startTransition(async () => {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(detail || data.error || "Failed to create post")
        }
  ...
  return (
    <div className="p-6 max-w-4xl space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-sm font-mono font-semibold text-[#e0e0e0]">New Post</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-[#0d0d0d] border border-[#1e1e1e] rounded-lg p-5 space-y-4">
          ...
          {/* Slug */}
          <div>
            <label className="block text-[11px] font-mono font-semibold text-[#555] uppercase tracking-wider mb-1.5">
              Slug
              {!slugManual && <span className="ml-2 text-[9px] text-[#444] normal-case">(auto-generated)</span>}
            </label>
            <input
              type="text"
              value={slug}
